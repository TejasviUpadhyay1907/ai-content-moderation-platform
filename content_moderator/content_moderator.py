...
DOWNLOAD_URL = "https://github.com/your-username/ai-content-moderation-platform/releases/download/"
MODEL_URLS = {
...
class ContentModerator:
    ...
    Can initialize 5 different model types from model type or checkpoint path:
        - original:
            model trained on data from the Toxic Comment
            Classification Challenge
        - unbiased:
            model trained on data from the Unintended Bias in
            Toxicity Classification Challenge
        - multilingual:
            model trained on data from the Multilingual
...
def bert_content_moderator():
    return load_model("original")


def albert_content_moderator():
    return load_model("original-small")


def roberta_content_moderator():
    return load_model("unbiased")


def albert_unbiased_moderator():
    return load_model("unbiased-small")


def xlmr_multilingual_moderator():
